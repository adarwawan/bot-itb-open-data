...
# from extractor.research.list_extractor.list_extractor import ListExtractor
...
class ResearchSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        with open('exclude_words.txt', 'r') as myfile:
            self.exclude_words = myfile.read().splitlines()
        ...
        for apath in response.xpath('//a'):
            for href in apath.xpath('@href').extract():
                ...
                if (len(text)) != 0:
                    ...
                    self.name = 'A'
                self.url = urlparse.urljoin(response.url, href)

            if self.violation_check(self.url, self.name):
                yield scrapy.Request(self.url, callback=self.parse, meta={'referer': response.url, 'name': self.name })
    ...
    def violation_check(self, url, text):
        ...
        pattern = re.compile('\.(pdf|jpe?g|docx?|xlsx?|pptx?|png|gif|zip|rar|swf)$', re.I)
        ...
        if len(set(self.exclude_words).intersection(words)) > 0:
            return False
