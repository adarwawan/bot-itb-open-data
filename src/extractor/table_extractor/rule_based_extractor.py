    ...
    def getCandidatesUnorderedList(self, body_soup):
        ...
        if len(list_element) > 0:
            ...
            for l in list_element:
                child_table = l.find_all('ul')
                if len(child_table) == 0:
                    # ciri-ciri bibliografi?
                    # selection ul
                    if (self.wordAvgLimit(l) and self.notContainNumberLimit(l)):
                        isList = True
                    if isList:
                        out_path = self.printTableToFile(l, headers, j, self.tableCount)
                        tables.append(out_path)
                        self.tableCount = self.tableCount + 1
                        isList = False
                j += 1
